...
func (h *Handler) Handle(ctx context.Context, event accountDomainUsers.EmailChangedDomainEvent) error {
	return h.transactionManager.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		return h.eventBusInboxService.EnsureOnce(txCtx, "notifications:account.user.email_changed", func(txCtx context.Context) error {
