...
func (h *Handler) Handle(ctx context.Context, command *MarkAllNotificationsSeenCommand) (*MarkAllNotificationsSeenResponse, error) {
	var updatedCount int64

	err := h.transactionManager.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		count, err := h.notificationRecipientRepository.MarkAllSeenByUserID(txCtx, command.UserID, command.OrganizationID, time.Now().UTC())
		if err != nil {
			return err
		}
		updatedCount = count
