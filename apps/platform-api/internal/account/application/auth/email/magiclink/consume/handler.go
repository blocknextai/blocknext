...
func (h *Handler) Handle(ctx context.Context, command *MagicLinkConsumeCommand) (*accountApplicationAuth.AccessTokenResponse, error) {
	...
	return h.tokenIssuer.IssueTokens(ctx, userID, accountDomain.AuthProviderEmail, command.IPAddress, command.UserAgent)
}
