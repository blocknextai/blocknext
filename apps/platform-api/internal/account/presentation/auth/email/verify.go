...
func NewVerifyHandler(handler cqrs.Handler[*verify.VerifyCommand, *accountApplicationAuth.AccessTokenResponse]) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		result, err := handler.Handle(c.RequestCtx(), &verify.VerifyCommand{
			Token:     request.Token,
			IPAddress: c.IP(),
