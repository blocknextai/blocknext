...
## Layout
Standard module layout: `domain/` (aggregates + rules), `application/` (aggregate services + event handlers), `usecases/` (one `Service` per aggregate, one file per use case), `postgres/` (persistence), `authproviders/` (OAuth provider adapters), `migrations/`, `http/` (routes + handlers). Wired in `module.go`.
