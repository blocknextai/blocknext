...
Read-side only (queries), each a method on its aggregate's `usecases/<aggregate>.Service` (nodes, credentials, adapters):
...
- **Bounded contexts:** `filegateway` (`FileGateway`, injected into media-capable node registrations); `common` (`domain/oauth2`, `domain/credential` mask constant). Other contexts (workflows, executions, taskrunner, mcp) consume this engine's exposed services/registries.
...
application/     read services (nodes, credentials, adapters, executors, mcp) + jsonschema validator
usecases/        services.go runs node/credential registration and builds the query Services (nodes, credentials, adapters)
http/            routes.go + fiber handlers (nodes, credentials, webhooks)
module.go        Module wiring: builds services + use cases, mounts http routes
```
