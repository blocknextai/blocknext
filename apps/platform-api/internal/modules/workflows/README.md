...
- **SSE streaming** — the HTTP handler streams the reply as `text/event-stream` (`event: message` chunks, `event: done` / `event: error` terminators); the completed assistant reply is saved back to the session.
...
Generation (`usecases/generation/*` + `application/generation/*`):
- `sessions_{createsession,updatesession,deletesession,getallsessions,getallsessionmessages}` — CRUD over generation sessions and their messages.
...
## Layout
Full module layout: `domain/` (workflows + generation/{sessions,messages}), `application/` (workflow service + generation chat/context builders), `usecases/` (workflows, generation), `postgres/` (repositories), `migrations/`, `http/` (workflows + generation routes), wired in `module.go`. `module.go` exposes `WorkflowService` for cross-context consumers.
