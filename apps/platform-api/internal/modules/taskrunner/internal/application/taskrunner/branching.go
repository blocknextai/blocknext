...
func branchReaches(store *OutputStore, taskID uuid.UUID, d *dag.DAG, parent *dag.Node, childID string) (bool, error) {
	...
	executor, ok := nodeengineContract.GetExecutor(parent.NodeID)
	if !ok {
		return true, nil
	}
	if _, routes := executor.(nodeengineContract.BranchingExecutor); !routes {
		return true, nil
	}
...
func routesItems(node *dag.Node) bool {
	executor, ok := nodeengineContract.GetExecutor(node.NodeID)
	if !ok {
		return false
	}
	_, routes := executor.(nodeengineContract.BranchingExecutor)
	return routes
}
