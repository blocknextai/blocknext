...
## Layout
Standard module layout: `domain/` (aggregates + rules), `application/` (aggregate service + validator), `usecases/apikeys/` (one file per use case), `postgres/apikeys/` (persistence), `migrations/`, `http/` (routes + handlers). Wired in `module.go`.
