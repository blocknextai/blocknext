...
- **Infrastructure:** `realtime.Broadcaster` (Redis pub/sub, subscribed per organization channel); `common/auth.AuthMiddleware` for the upgrade handshake. No database.

## Layout
`application/` (hub), `domain/` (connections, rooms), and `http/` (websockets) are present and wired in `module.go`; there is no persistence layer. `module.go` also exposes `Shutdown()` for graceful teardown of the hub.
