...
## What it does
Loads the platform configuration, bootstraps `Core` (DB, cache, secret manager, file gateway, event bus) and the full module graph via `bootstrap.NewPlatformAPI`, then builds a Fiber app and registers every module's routes behind JWT auth, API-key, and cache middleware. It serves liveness/readiness probes and applies a Redis-backed rate limiter. When `TASK_RUNNER_MODE=embedded` it also starts the task runner (`StartAsMain`) and the eventbus transactional-outbox relay in the same process.
...
- **Runs as:** HTTP server on `HTTP_SERVER_*` address (Fiber).
...
- taskrunner, apikeys, notifications, ws, webhooks
...
- Graceful shutdown via `bootstrap.ListenAndWait`: cancels app context, then shuts Fiber, app modules, and core in order.
