...
// ConditionalChildren returns the children to traverse from the given node. For
// condition nodes it selects the branch matching the boolean status in
// nodeResult; for other nodes it returns all direct children.
func (d *DAG) ConditionalChildren(nodeID string, nodeResult map[string]any) []string {
	var conditionalChildren []string

	node := d.NodeByID(nodeID)
	if node != nil && node.NodeID == "condition" {
		if nodeResult == nil {
			return conditionalChildren
		}

		statusValue, exists := nodeResult["status"]
		if !exists {
			return conditionalChildren
		}

		boolResult, ok := statusValue.(bool)
		if !ok {
			switch v := statusValue.(type) {
			case string:
				boolResult = v == "true" || v == "1"
			case int:
				boolResult = v != 0
			case int64:
				boolResult = v != 0
		...
		edges := d.NodeEdges(nodeID)
		for _, edge := range edges {
			if edge.Condition != "true" && edge.Condition != "false" {
				continue
			}

			if (boolResult && edge.Condition == "true") || (!boolResult && edge.Condition == "false") {
				conditionalChildren = append(conditionalChildren, edge.Target)
			}
		}
	} else {
		conditionalChildren = d.NodeChildren(nodeID)
	}

	return conditionalChildren
}

// IsConditionalNode reports whether the node with the given ID is a condition node.
func (d *DAG) IsConditionalNode(nodeID string) bool {
	node := d.NodeByID(nodeID)
	return node != nil && node.NodeID == "condition"
}
